...
staticni void send_output_events(Oosc_dev *oosc_dev, Midi_mode *midi_mode,
  ...
  // REFACTOR: First pass - Process all events with memory barriers between channels
  for (Usz i = 0; i < count; ++i) {
    Oevent const *e = events + i;
#ifdef __linux__
    __sync_synchronize(); // Memory barrier between events
#endif
  ...
  // REFACTOR: Second pass - Send MIDI events with channel synchronization
  if (midi_note_count > 0) {
#ifdef __linux__
    __sync_synchronize(); // Memory barrier before note processing
#endif
  ...
  // REFACTOR: Third pass - Handle mono notes with additional synchronization
  if (monofied_chans) {
#ifdef __linux__
    __sync_synchronize(); // Memory barrier before mono notes
#endif
    ...
    // MIDI note on sent, followed immediately by a MIDI note off.
    Usz start_note_offs, end_note_offs;
    ...
    for (Usz chan = 0; chan < 16; ++chan) {
      ...
      if (midi_note_count < Midi_on_capacity) {
        new_susnotes[midi_note_count] =
            (Susnote){.remaining = (float)(frame_secs * (double)mmo->duration),
                      .chan_note = (U16)((chan << 8u) | mmo->note_number)};
        ++midi_note_count;
      }
    }
